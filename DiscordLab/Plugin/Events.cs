...
namespace DiscordLab
{
        ...
        IEventHandlerRoundStart, IEventHandlerWarheadCancel, IEventHandlerWarheadDetonate, IEventHandlerWarheadStart
    {
        ...
        public void OnPlayerEscape(PlayerEscapeEvent ev) => DiscordLab.bot.NewMessage($"{ev.Player.nameClean} escaped the facility and became {ev.newRole}");
        public void OnPlayerJoin(PlayerJoinEvent ev) => DiscordLab.bot.NewMessage($"**{ev.Player.nameClean} ({ev.Player.userId} from ||~~{ev.Player.ipAddress}~~||) has joined the server**");
