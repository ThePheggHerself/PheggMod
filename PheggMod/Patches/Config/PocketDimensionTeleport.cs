...
namespace PheggMod.Patches
{
	...
	public class PMPocketDimensionTeleport : PocketDimensionTeleport
	{
		...
		private void OnTriggerEnter(Collider other)
		{
			...
			{
				PlayerStats.HurtPlayer(new PlayerStats.HitInfo(999990f, "WORLD", DamageTypes.Pocket, 0), other.gameObject, true, true);
				return;
			}

			if (type == PDTeleportType.Killer || rand.Next(0, 100) < 40)
			{
				float dmg = Mathf.Clamp(((PlayerStats.Health + (int)PlayerStats.syncArtificialHealth) / 5), 5, 50);
				PlayerStats.HurtPlayer(new PlayerStats.HitInfo(dmg, "POCKET", DamageTypes.Pocket, 0), other.gameObject, true, true);
		...
		private void RemoveCorrosionEffect(GameObject escapee)
		{
			...
			escapee.GetComponentInParent<PlayerEffectsController>().GetEffect<Corroding>().ServerDisable();
		}
	}
}
